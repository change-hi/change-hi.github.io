...
The data contains over 20000 individual samples. To analyze the data we are going to clean it up. Then, in the next module we will analyze and visualize it. To do this we will be using some of knowledge we have already learned while also introducing some new things. **Note:** The dataset we are using has been modified from its original format to reduce the amount of cleaning up we need to do.
...
<details>
  ...
  To get the name of the columns we can simply access the `columns` `Series` attribute that every `DataFrame` has. This can be done either through chaining it after our `read_csv` method or by store it in a variable and then calling the method using that variable. Below we use the chain approach.
